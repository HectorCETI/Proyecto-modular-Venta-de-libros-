...
$query_clicks = "
SELECT libros.id, libros.nombre, COUNT(libro_clicks.libro_id) as clics
FROM libro_clicks
...
SELECT libros.id, libros.nombre, COUNT(libro_busquedas.libro_id) as busquedas
...
// Procesamiento de datos para recomendaciones
$book_scores = [];

// Agregar puntuaciones según clics
foreach ($result_clicks as $click) {
    $book_id = $click['id'];
    if (!isset($book_scores[$book_id])) {
        $book_scores[$book_id] = 0;
    }
    $book_scores[$book_id] += $click['clics'] * 0.4; // Peso para clics
}

// Agregar puntuaciones según búsquedas por nombre
foreach ($result_busqueda_nombre as $busqueda) {
    $book_id = $busqueda['id'];
    if (!isset($book_scores[$book_id])) {
        $book_scores[$book_id] = 0;
    }
    $book_scores[$book_id] += $busqueda['busquedas'] * 0.4; // Peso para búsquedas por nombre
}

// Agregar puntuaciones según búsquedas por ID
foreach ($result_busqueda_id as $busqueda) {
    $book_id = $busqueda['id'];
    if (!isset($book_scores[$book_id])) {
        $book_scores[$book_id] = 0;
    }
    $book_scores[$book_id] += $busqueda['busquedas'] * 0.2; // Peso para búsquedas por ID
}

// Ordenar los libros por su puntuación en orden descendente
arsort($book_scores);

// Obtener los top 4 libros recomendados
$recommended_books_ids = array_slice(array_keys($book_scores), 0, 4);

// Guardar recomendaciones en un archivo JSON para usarlas en el index
file_put_contents('../../recomendaciones.json', json_encode($recommended_books_ids));
