...
$recomendaciones_ids = json_decode(file_get_contents('recomendaciones.json'), true);

// Obtener detalles de los libros recomendados desde la base de datos
$recomendaciones = [];
if (!empty($recomendaciones_ids)) {
    $placeholders = implode(',', array_fill(0, count($recomendaciones_ids), '?'));
    $query = "SELECT * FROM libros WHERE id IN ($placeholders)";
    $stmt = $conexion->prepare($query);
    $stmt->execute($recomendaciones_ids);
...
<div class="container-fluid p-0">
    <div class="container mt-4">
        ...
        <!-- Libros Recomendados -->
        <h2 class="mb-4">Libros Recomendados</h2>
        <div class="row">
            <?php foreach ($recomendaciones as $libro) { ?>
